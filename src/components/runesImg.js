    ...
    runesJSON.forEach(style => {
        ...
        style["slots"].forEach(runes => {
            runes["runes"].forEach(rune => {
                runePathImg[rune["id"]] = rune["icon"].replace("perk-images/Styles/", "");
            })
        })
    });
...
const runeImg = importAll(require.context("../assets/loldata/img/perk-images/Styles", true, /\.(png)$/));
...
const emblemesImgs = importAll(require.context("../assets/loldata/current/img/emblem/", false, /\.(png)$/));
...
const passiveImgs = importAll(require.context("../assets/loldata/current/img/passive/", false, /\.(png)$/));

export const getPassiveImg = (id) => {
    return passiveImgs[`${id}.png`];
}

const spellsImgs = importAll(require.context("../assets/loldata/current/img/spell/", false, /\.(png)$/));
...
export const initializeTree = (tree, type, number, getImgFunction, arrayFct = false) => {
    let rate = undefined;
    ...
    for(let i = 0; i < number; i++){
        arrayType[i] = {
            "id" : tree[type][i],
            "img" : arrayFct ? getImgFunction[tree[type][i]] : getImgFunction(tree[type][i])
        };
    }
