            ...
            <div>
                   {
                        itemsIconsFirstRow.map((itemIcon, index) => {
                            if(props.idItems[index] === undefined){
                                return <div key={`${props.idItems[index]}${props.identifier}`} className="empty-item"></div>
                            }
                            else{
                                return <GameHistoryCardTooltip key={`${index}${props.identifier}`} type="items" itemsJSON={itemsJSON} idItems={props.idItems} itemIcon={itemIcon} identifier={props.identifier} index={index} /> 
                            }
                    })
                    } 
                </div>
                <div>
                    {
                        itemsIconsSecondRow.map((itemIcon, i) => {
                            ...
                            if(props.idItems[index] === undefined){
                                return <div key={`${index}${props.identifier}`} className="empty-item"></div>
                            }
