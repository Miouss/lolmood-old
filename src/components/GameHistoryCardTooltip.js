...
function GameHistoryCardTooltip(props){
    switch(props.type){
        ...
        case "items" :
            return setItemsToolTip(props.itemsJSON, props.idItems, props.itemIcon, props.identifier, props.index)
        default:
    }
}
...
function setRunesTooltip(runeJSON, idRunes, runeIcon, index, row){
    ...
    return (
        <div data-tip data-for={`${idRunes[index]}${row}`}>
            <ReactTooltip className="tooltip-container" id={`${idRunes[index]}${row}`} effect="solid" data-place="bottom">
                ...
                <div dangerouslySetInnerHTML={{__html : runeDescription}} >
...
function setPerkToolTip(runeJSON, idPerk, perkIcon){
    ...
    return(
        <div className="perk" key={`${idPerk}`} data-tip data-for={`${idPerk}`}>
            <ReactTooltip class="tooltip-container" id={`${idPerk}`} effect="solid" data-place="bottom">
                <div className="tooltip-title">
                ...
                <div dangerouslySetInnerHTML={{__html : perkDescription}} >
...
function setItemsToolTip(itemsJSON, idItems, itemIcon, identifier, index){
    return(
        <article data-tip data-for={`${identifier}${index}`}>
            <ReactTooltip className="tooltip-container" id={`${identifier}${index}`} effect="solid" data-place="bottom">
                <div className="tooltip-title">
                    <img src={itemIcon} alt={`item${index}`} />
                    <h6>{itemsJSON["data"][idItems[index]]["name"]}</h6>
                </div>
                <div dangerouslySetInnerHTML={{__html : itemsJSON["data"][idItems[index]]["description"]}} >
